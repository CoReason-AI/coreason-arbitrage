...
def test_smart_client_retry_loop_accumulates_failures(mock_engine: ArbitrageEngine) -> None:
    ...
    # Spy on LoadBalancer
    mock_engine.load_balancer.record_failure = MagicMock(wraps=mock_engine.load_balancer.record_failure)  # type: ignore

    # Mock Router.route to return A then B
    # Note: We need to spy on the *instance* of Router inside the client,
    # or patch the router logic.
    # Since client.router is created in __init__, we can mock its route method directly.
    ...
    with patch("coreason_arbitrage.smart_client.completion") as mock_completion:
        ...
        mock_engine.load_balancer.record_failure.assert_called_with("provider-a")  # type: ignore
...
def test_smart_client_does_not_record_failure_for_bad_request(mock_engine: ArbitrageEngine) -> None:
    ...
    with patch("coreason_arbitrage.smart_client.completion") as mock_completion:
        ...
        mock_engine.load_balancer.record_failure.assert_not_called()  # type: ignore
